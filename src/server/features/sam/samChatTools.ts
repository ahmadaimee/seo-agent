...
import seoAgentFactSheet from "@/server/features/sam/seoagent-fact-sheet.md?raw";
...
// Adapt one S.E.O Agent tool into an AI SDK tool. The shared handler receives the
 ...
 * free site-reading tools. Every tool the S.E.O Agent MCP server exposes is
...
export function buildSamMcpTools(
  ...
  return {
    ...
    get_product_info: tool({
      description:
        "The S.E.O Agent fact sheet: what the product does, plans/pricing, credit costs, integrations, MCP setup. Call before answering questions about S.E.O Agent itself. Uses no credits.",
      inputSchema: z.object({}),
      execute: () => Promise.resolve({ factSheet: seoAgentFactSheet }),
    }),
