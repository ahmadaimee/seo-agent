...
import { z } from "zod";
import type { createSeoAgentOAuthProvider } from "./oauth-provider";
...
vi.mock("@/server/mcp/transport", () => ({
  handleAuthenticatedSeoAgentMcpRequest: vi.fn(),
}));
...
async function dispatch(
  provider: ReturnType<typeof createSeoAgentOAuthProvider>,
  request: Request,
) {
...
describe("S.E.O Agent OAuth provider configuration", () => {
  beforeEach(() => {
  ...
  it("binds tokens and protected-resource metadata to the canonical MCP URL", async () => {
    const { createSeoAgentOAuthProvider } = await import("./oauth-provider");
    const provider = createSeoAgentOAuthProvider(() => new Response("app"));

    await dispatch(provider, new Request("http://localhost:3010/health"));

    expect(mocks.options).toHaveLength(1);
    expect(mocks.options[0]?.resourceMetadata).toEqual({
      resource: "http://localhost:3010/mcp",
      scopes_supported: ["mcp"],
      resource_name: "S.E.O Agent MCP",
    });
  ...
  it("lets the provider issue Perplexity a real client secret", async () => {
    ...
    await dispatch(
      provider,
      new Request("http://localhost:3010/api/auth/oauth2/register", {
        method: "POST",
  ...
  it("includes the authorization-server issuer when consent is denied", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("http://localhost:3010/api/oauth/consent", {
        method: "POST",
        headers: {
  ...
  it("redirects safe authorization errors with state and issuer", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("http://localhost:3010/api/auth/oauth2/authorize"),
      {
        OAUTH_PROVIDER: {
          parseAuthRequest: () =>
            Promise.reject(
