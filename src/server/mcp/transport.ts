...
import { createSeoAgentMcpServer } from "@/server/mcp/server";
...
async function handleLegacyJsonRequest(request: Request, props: McpProps) {
  ...
  // no S.E.O Agent tool issues them.
  const server = createSeoAgentMcpServer(props);
...
function createRequestHandler(
  ...
  const modernHandler = createMcpHandler(() => createSeoAgentMcpServer(props), {
    route: MCP_ROUTE,
    allowedOriginHostnames,
    legacy: "reject",
...
export async function handleAuthenticatedSeoAgentMcpRequest(
  request: Request,
  props: unknown,
  env: unknown,
...
export async function handleSelfHostedSeoAgentMcpRequest(
  request: Request,
