...
type Provider = ReturnType<typeof createSeoAgentOAuthProvider>;
...
beforeEach(async () => {
  ...
  provider = createSeoAgentOAuthProvider(() => new Response("app"));
...
async function callMcp(accessToken: string) {
  ...
  expect(response.status).toBe(200);
  return propsEchoSchema.parse(await response.json()).props.seoAgentAuth;
}
