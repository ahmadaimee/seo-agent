...
import type { handleAuthenticatedSeoAgentMcpRequest } from "@/server/mcp/transport";
...
const mocks = vi.hoisted(() => ({
  ...
  handleAuthenticatedSeoAgentMcpRequest:
    vi.fn<typeof handleAuthenticatedSeoAgentMcpRequest>(),
}));
...
vi.mock("@/lib/auth", () => ({
  ...
  getHostedBaseUrl: () => "http://localhost:3010",
}));
...
vi.mock("@/server/mcp/transport", () => ({
  handleAuthenticatedSeoAgentMcpRequest:
    mocks.handleAuthenticatedSeoAgentMcpRequest,
}));
...
function request(headers?: HeadersInit, method = "POST") {
  return new Request("http://localhost:3010/mcp", { method, headers });
}
...
describe("handleMcpApiKeyRequest", () => {
  beforeEach(() => {
    ...
    mocks.handleAuthenticatedSeoAgentMcpRequest.mockResolvedValue(
  ...
  it("handles a valid key with the hosted user, organization, and MCP scopes", async () => {
    ...
    expect(mocks.handleAuthenticatedSeoAgentMcpRequest).toHaveBeenCalledTimes(1);
    const [passedRequest, props, passedEnv, passedCtx] =
      mocks.handleAuthenticatedSeoAgentMcpRequest.mock.calls[0];
  ...
  it("returns 403 when the user has no organization membership", async () => {
    ...
    expect(mocks.handleAuthenticatedSeoAgentMcpRequest).not.toHaveBeenCalled();
  });
  ...
  it("leaves non-S.E.O Agent bearer tokens for OAuth", async () => {
    await expect(
  ...
  it("leaves non-S.E.O Agent x-api-key values for OAuth", async () => {
    await expect(
      handleMcpApiKeyRequest(
        request({
