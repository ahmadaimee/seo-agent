...
type SeoAgentToolDefinition<Input extends ToolSchema> = {
  name: string;
  config: {
    title?: string;
    description?: string;
...
function registerSeoAgentTool<Input extends ToolSchema>(
  server: McpServer,
  tool: SeoAgentToolDefinition<Input>,
  authProps: McpProps,
) {
...
export function createSeoAgentMcpServer(authProps: McpProps) {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        "S.E.O Agent research tools use credits. Proceed with normal focused research, but ask the user for confirmation before planned batches over 2,000 credits.",
    },
  );

  const register = <Input extends ToolSchema>(
    tool: SeoAgentToolDefinition<Input>,
  ) => registerSeoAgentTool(server, tool, authProps);
