...
import { handleAuthenticatedSeoAgentMcpRequest } from "@/server/mcp/transport";
...
export type SeoAgentOAuthEnv = Env & {
...
function getOAuthHelpers(env: SeoAgentOAuthEnv) {
...
function createDefaultHandler(
  appFetch: AppFetch,
): ExportedHandlerWithFetch<SeoAgentOAuthEnv> {
  return {
...
const mcpApiHandler: ExportedHandlerWithFetch<SeoAgentOAuthEnv> = {
  async fetch(request, env, ctx) {
    return handleAuthenticatedSeoAgentMcpRequest(request, ctx.props, env, ctx);
  },
};

function createProvider(appFetch: AppFetch, resource: string) {
  const options: OAuthProviderOptions<SeoAgentOAuthEnv> = {
    apiRoute: MCP_ROUTE,
...
export function createSeoAgentOAuthProvider(appFetch: AppFetch) {
  ...
  let provider: OAuthProvider<SeoAgentOAuthEnv> | undefined;
  const getProvider = () =>
  ...
  return {
    ...
    purgeExpiredData(env: SeoAgentOAuthEnv) {
