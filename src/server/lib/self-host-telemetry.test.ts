...
describe("maybeSendSelfHostHeartbeat", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("SEO_AGENT_TELEMETRY_DISABLED", "");
  ...
  it("does not send when SEO_AGENT_TELEMETRY_DISABLED is set", async () => {
    vi.stubEnv("SEO_AGENT_TELEMETRY_DISABLED", "1");
    const harness = createHarness();
  ...
  it('still sends when the disable flags are explicitly "0"/"false"', async () => {
    vi.stubEnv("SEO_AGENT_TELEMETRY_DISABLED", "0");
