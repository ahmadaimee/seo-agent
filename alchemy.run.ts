...
// Preview hostnames are `seo-agent-<stage>.<WORKERS_SUBDOMAIN>` — the naming
...
// - Stage "hosted-prod": names the EXISTING github.com/ahmadaimee/seo-agent production resources
  ...
  .object({
    ...
    workflows: z.array(
      z.object({
        ...
        name: z.string(),
        class_name: z.string(),
        // Set when the workflow class lives in the seo-agent-audit aux
...
const makeResources = (stage: string) => {
  ...
  return {
    DB: Cloudflare.D1.Database("DB", {
      name: prod ? PROD_NAMES.d1 : `seo-agent-db-${stage}`,
    ...
    R2: Cloudflare.R2.Bucket("R2", {
      name: prod ? PROD_NAMES.r2 : `seo-agent-r2-${stage}`,
    ...
    KV: Cloudflare.KV.Namespace("KV", {
      title: prod ? PROD_NAMES.kv : `seo-agent-kv-${stage}`,
    }).pipe(keep),
    OAUTH_KV: Cloudflare.KV.Namespace("OAUTH_KV", {
      title: prod ? PROD_NAMES.oauthKv : `seo-agent-oauth-kv-${stage}`,
    }).pipe(keep),
  };
};
  ...
  Effect.gen(function* () {
    ...
    if (!policyAud) {
      ...
      const application = yield* emailAccessGate({
        ...
        policyName: `seo-agent ${stage} self-host users`,
        applicationName: `seo-agent ${stage}`,
...
const dataEnv = {
  ...
  SEO_AGENT_TELEMETRY_DISABLED: optionalVar("SEO_AGENT_TELEMETRY_DISABLED"),
};
...
export default Alchemy.Stack(
  ...
  Effect.gen(function* () {
    ...
    const auditWorker = yield* Cloudflare.Worker("seo-agent-audit", {
      name: `${workerName(stage)}-audit`,
      main: "./dist/seo_agent_audit/index.js",
      bundle: false,
      url: false,
      compatibility: {
    ...
    const app = yield* Cloudflare.Worker("seo-agent", {
      ...
      // Site audits moved to the seo-agent-audit worker, but RankCheckWorkflow
      ...
      env: {
        ...
        // privately in the seo-agent-audit worker). Alchemy backs new DO
      ...
      // Prod adopts the live worker serving app.github.com/ahmadaimee/seo-agent; never delete it
