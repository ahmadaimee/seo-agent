...
Sources, in the order to prefer them. Say which one produced every number.

| Source | Gives | Cost |
| --- | --- | --- |
| Google Search Console (`/seo google`) | the user's own linking sites and linked pages | free, first-party |
| Common Crawl web graph | domain-level PageRank and harmonic centrality | free |
| Moz API | Domain Authority, Page Authority, spam score (2,500 rows/month free) | free tier |
| Bing Webmaster Tools | referring pages and anchors for verified sites | free |
| Keywords Everywhere (Open PageRank) | a single 0-10 domain rank fallback | free signup |
| S.E.O Agent MCP `get_backlinks_overview` | referring-domain profile and backlink counts for any domain, including competitors | paid (credits) |
| DataForSEO extension | full backlink rows with spam scores | paid |

`get_backlinks_overview` is the fastest way to profile a **competitor's** links,
since the free sources mostly cover domains the user controls. Run `whoami` for the
credit balance first, and check the research log before re-buying. Hand the
prospecting work that follows to `seo-link-prospecting`.

Backlink data may be unavailable on any given source. Continue with the others and
say so rather than scoring what you did not measure.
