---
name: setup-seoagent
description: Set up S.E.O Agent in the current AI agent. Use when a user pastes the S.E.O Agent installation prompt or asks to connect its plugin, MCP, and skills.
metadata:
  internal: true
---

Set up S.E.O Agent in this agent. Do what you can; guide me through anything that needs my input.

## 1. Check this agent

- Identify this agent and its version. Ask only if you cannot tell.
- Check for an existing S.E.O Agent connection. Preserve other integrations and avoid duplicates.

## 2. Install the plugin first

The official plugin bundles MCP + SEO skills, with S.E.O Agent namespacing and shared updates.

- **Codex:** follow the [plugin guide](https://github.com/ahmadaimee/seo-agent/blob/main/docs/codex-plugin).
- **Claude Code:** follow the [plugin guide](https://github.com/ahmadaimee/seo-agent/blob/main/docs/claude-code-plugin).
...
For self-hosted S.E.O Agent, use its endpoint directly; the official plugin targets the hosted service.
...
**Next**
1. `[Give the native reload command or UI action for this agent, only if needed.]` Then say “Check that S.E.O Agent is connected.” Approve sign-in if prompted.
2. Try one of these:
