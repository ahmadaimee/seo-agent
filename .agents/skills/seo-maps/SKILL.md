...
### Workflow (S.E.O Agent MCP, preferred)

`get_local_rank_grid` returns the rank at every point of a grid around a
coordinate, with each point's result count and its #1 business, in one call.

1. Resolve the business and its coordinate from a `search_local_businesses` or
   `get_local_serp_results` row (see `seo-local`). Confirm the coordinate matches
   the storefront before spending -- a grid centred on the wrong place is worse
   than no grid.
2. Choose the grid size. 3x3 is nine searches; only go to 5x5 when the service
   area is genuinely wide.
3. **Display the cost estimate and ask for confirmation** (below).
4. Call `get_local_rank_grid` for the main keyword.
5. Read each point's `topResult` to name who wins where the target does not.
6. Calculate SoLV: `(top_3_count / total_points) * 100`
7. Render the ASCII heatmap in the output.

A missing rank at a grid point means the business was not among the results
returned there. Read it together with that point's `resultsCount`: a full result
set means outranked; a near-empty one means a sparse SERP, not proof of
invisibility.

### Workflow (DataForSEO extension)
...
Every grid point is a paid SERP call. Never run a 5x5 grid, or grids for several
keywords, without telling the user the cost first.
