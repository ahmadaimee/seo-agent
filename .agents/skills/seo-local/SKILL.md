...
Everything above works from the page HTML alone. Live listing data makes the
comparison against real competitors possible. Use the first source available.

### 1. S.E.O Agent MCP (preferred)

It carries the shared project context, the research log and credit reuse. Read
`docs/PROJECT-CONTEXT.md` first; this skill needs `business_overview` (what the
business does and where it operates decides which keywords and radius matter).

| Tool | What it gives you |
| --- | --- |
| `search_local_businesses` | nearby listings filterable by `minRating`, `minReviews`, `isClaimed`. One call with the brand name as `query` and a wide radius returns category, rating, review count, claimed status, coordinates and `cid` for every location of a chain -- usually enough that per-location profile calls are unnecessary. Pass `isClaimed: false` to find unclaimed listings when prospecting. |
| `get_local_serp_results` | the Maps / Local Finder result set near a coordinate. Rows carry `cid` and `place_id` -- collect them once and reuse them. |
| `get_business_profile` | the full profile for one business (hours, rating breakdown) when the search row is not enough. |
...
| `list_business_categories` | valid category slugs. |

Required inputs: the business name or a `cid` / `placeId` (most reliable), its
coordinate -- derive it from a `search_local_businesses` or
`get_local_serp_results` row rather than asking -- and one to three keywords
customers actually search ("emergency plumber", not the brand name).

**Live comparison workflow**

1. Find the business. Given only a name or website, `search_local_businesses`
   (name as `query`, wide radius) locates the listing and yields its `cid` and
   coordinate. Several locations means it is a chain -- see below.
2. Run `get_local_serp_results` for the main keyword near that coordinate. Record
   the top 3-5 competitors' `cid` / `place_id` and the user's own row.
3. Compare the listing against the top two competitors: primary category,
   additional categories, review count, hours completeness, photo count, claimed
   status.
4. Sanity-check each listing's website link (`url` / `contact_url`): it should
   deep-link to that location's page on the project domain, not a homepage or a
   stale domain.
5. Call `get_business_reviews` for the user and the strongest competitor. Look at
   volume, recency, average rating and how many reviews got an owner reply.
6. Hand off to `seo-maps` for the geo-grid when you need to separate "ranks at the
   storefront only" from "ranks across the service area".
7. Add `get_google_business_questions` and `get_business_updates` when the profile
   basics are already competitive and the gap is engagement rather than setup.
8. Turn the evidence into a prioritized list. Category and claim problems outrank
   posting cadence every time.
...
covering categories, reviews (count, recency, owner replies), hours and profile
completeness, listing website links, Maps coverage, Q&A and posting hygiene.

**Multi-location businesses.** Always build the snapshot table for the whole chain
-- one `search_local_businesses` call covers it. The per-location deep dives
(reviews, grid, posts, Q&A) are where cost scales. Five locations or fewer: deep
dive them all. More than five: present the snapshot table, then ask the user which
one to three locations to deep dive, recommending sensible defaults such as the
weakest profile in the densest market.

On finish, write back the local competitors that have a website via
`addCompetitors` (competitor rows are keyed by domain, so skip listings without
one), a corrected `business_overview`, and a research log entry:
`{ appendResearchLog: { summary: "Local SEO: <business> near <area>. Verdict: <conclusion>" } }`.

### 2. DataForSEO extension

If the DataForSEO MCP is connected instead, use
`business_data_business_listings_search` for live GBP and business-listing data
and citation auditing across directories, and `serp_organic_live_advanced` for
real-time local pack positions.

### Guardrails for live local data

- Match businesses by `cid` or `place_id` whenever you have one. Name matching
...
- Check the research log before re-buying research that ran in the last 30 days.
