...
name: seo-cluster
description: >
  SERP-based semantic topic clustering and keyword-to-page mapping. Groups keywords
  by actual Google SERP overlap and by intent (not text similarity), maps each
  cluster to an existing or proposed page, detects cannibalization from Search
  Console, designs hub-and-spoke architecture with internal link matrices, and
  generates interactive visualizations. Optionally executes content creation if
  claude-blog is installed. Use when user says "topic cluster", "content cluster",
  "semantic clustering", "keyword mapping", "map keywords to pages",
  "cannibalization", "pillar page", "hub and spoke", "content architecture",
  "keyword grouping", or "cluster plan".
user-invocable: true
...
| `/seo cluster map-pages <keywords>` | Map an existing keyword list to existing or proposed pages |

---

## Keyword Sourcing

Clustering needs a candidate set. Use the richest source available.

### With the S.E.O Agent MCP connected (preferred)

Read `docs/PROJECT-CONTEXT.md` first. This skill needs **key pages** -- they are
the existing pages clusters map onto. If none are saved, propose a shortlist from
the site, an audit or Search Console, confirm it, and write it back with
`addKeyPages` before continuing.

| Tool | Use |
| --- | --- |
| `get_search_console_performance` | start from real first-party demand. Request `dimensions: ["query","page"]` to get the queries already earning impressions **and** the pages earning them. This is the best possible input: it is free, it is the user's own data, and it exposes cannibalization directly. |
| `list_saved_keywords` | reuse an existing saved set, optionally filtered by tag |
| `research_keywords` | expand a seed topic when the user starts from a theme |
| `get_ranked_keywords` | exact ranking keyword and URL rows when starting from a domain or page |
| `get_keyword_metrics` | hydrate up to 700 known keywords with volume, difficulty, intent and CPC before prioritizing |
| `get_serp_results` | validate borderline pairs by SERP overlap (see Step 2) |
| `get_local_serp_results` | when Maps or local-pack intent decides whether terms belong on location pages |
| `save_keywords` | tag the final clusters -- only after explicit user confirmation |

Check the research log before spending. On finish write back new or corrected
`addKeyPages` entries with the topic each page now targets, plus a research log
entry: `{ appendResearchLog: { summary: "Keyword clustering: <keyword set>. Verdict: <conclusion>" } }`.

### Without the MCP

Expand from the seed with WebSearch as described in Step 1. Volume and difficulty
will be unavailable -- write `unknown` rather than inventing numbers, and lean
harder on SERP overlap, which WebSearch can still establish.

Clean the set before clustering: remove duplicates, irrelevant terms, branded-only
terms, and terms that clearly require a different product or audience.
...
## Page Mapping and Cannibalization

Clustering that stops at groups is unfinished. Every cluster must land on a page.

### Assign each cluster

- **Existing URL** when a saved key page or ranking URL already serves the intent.
- **New page** when nothing fits. Say which page type and where it sits in the
  hub-and-spoke architecture from Step 4.
- **Do-not-target / later** when the cluster is weak, off-strategy, or needs a
  product the user does not have. Say which, and why.

Label target pages as *proposed* whenever existing URL data was unavailable.

### Detect cannibalization

Two pages competing for one intent split clicks and confuse Google.

1. Flag any case where more than one existing page would target the same cluster.
2. When Search Console is connected, confirm it from real data:
   `get_search_console_performance` with `dimensions: ["query","page"]`, then look
   for a single query sending impressions to several URLs.
3. Recommend one of: consolidate (301 the weaker into the stronger), differentiate
   (re-target one page to a neighbouring intent), or canonicalize.

### Mapping table

| Cluster | Primary keyword | Secondary keywords | Intent | Target page | New or existing | Priority | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |

Open the output with the mapping summary: number of clusters, pages to create,
existing pages to update, and cannibalization or consolidation issues found.

For each cluster, include the recommended page brief: page type, the searcher's
problem, required sections, internal-link opportunities, and the save or tag
suggestion. Hand off to `seo-content-brief` for a full brief on any single page.

### Guardrails

- Do not over-cluster tiny sets. Under 10 usable terms, produce a simple map.
- Lexical similarity is not a cluster. SERP intent wins.
- Do not call `save_keywords` or replace tags broadly without explicit confirmation.
...
| `keyword-map.md` | Cluster-to-page mapping table with cannibalization findings |
...
| `seo-keywords` | Upstream: discovery and prioritization of the keyword set this skill clusters |
| `seo-content-brief` | Downstream: full brief for any page the mapping proposes |
