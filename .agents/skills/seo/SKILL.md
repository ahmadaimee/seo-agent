---
name: seo
description: "Comprehensive SEO for any website or business type. Site audits, single-page analysis, technical SEO (crawlability, indexability, Core Web Vitals with INP), schema, content quality (E-E-A-T), images, sitemaps, GEO for AI Overviews/ChatGPT/Perplexity, plus keyword research, competitor and market intelligence, rank tracking and link prospecting from a connected S.E.O Agent instance. Industry detection for SaaS, e-commerce, local, publishers, agencies. Triggers on: SEO, audit, schema, Core Web Vitals, sitemap, E-E-A-T, AI Overviews, GEO, technical SEO, content quality, page speed, keyword research, competitors, rank tracking."
user-invocable: true
...
e-commerce, publishers, agencies). Orchestrates 30 sub-skills and 18 sub-agents.
A separate optional Firecrawl extension is also installable (see "Optional
Extensions" below).

Two halves, one plugin:

- **Local analysis** runs on the bundled Python toolchain (crawler, headless
  Chromium, schema, content, images, free backlink sources). No account, no
  credits, works offline against any site.
- **Search data** comes from a connected S.E.O Agent MCP instance (keyword volume
  and difficulty, live SERPs, rank tracking, Maps grids, third-party backlink and
  domain estimates) and from the shared project context every skill reads and
  writes. See `docs/PROJECT-CONTEXT.md`.

Every command degrades gracefully: when the MCP is not connected, run the local
analysis and say which sections were skipped for lack of a data source.
...
| `/seo cluster <seed-keyword>` | SERP-based semantic clustering, keyword-to-page mapping, cannibalization |
| `/seo keywords <seed or domain>` | Keyword research: volume, difficulty, intent, SERPs, rank tracking (MCP) |
| `/seo competitor <domain>` | Deep dive on one competitor's organic footprint (MCP) |
| `/seo landscape <topic or domain>` | Market-level view: who wins, winning themes, gaps (MCP) |
| `/seo link-prospecting <url or asset>` | Find link prospects, contact paths, draft outreach (MCP) |
| `/seo project-setup [domain]` | Interview once, save scope, goals, positioning, competitors, key pages |
| `/seo coach` | Beginner-friendly coach mode: explains workflows and picks the next step |
...
0. If the S.E.O Agent MCP is connected, resolve the project and read
   `get_project_context` before anything else -- the business, goal and saved key
   pages decide which findings matter (see `docs/PROJECT-CONTEXT.md`). If the
   context is empty, suggest `/seo project-setup` after the audit, not before it.
...
16. If the MCP is connected, write the durable findings back to project context and
    append a research log entry so the next session does not re-buy the same research
...
Several paths can answer the same question. Use them in this order and say which
one produced each number.

1. **The user's own data** -- Google Search Console and GA4 via `/seo google`, or
   `get_search_console_performance` through the MCP. Free, first-party, always
   beats an estimate.
2. **The bundled Python toolchain** -- crawling, rendering, Core Web Vitals (lab),
   schema, content, images, Common Crawl. Free, no credits, no account.
3. **S.E.O Agent MCP** -- keyword metrics, live SERPs, rank tracking, Maps grids,
   backlink and domain estimates, and the shared project context. Paid per call
   (DataForSEO-backed): run `whoami` for the credit balance before spending, and
   check the research log before re-buying anything from the last 30 days.
4. **Optional extensions** -- DataForSEO direct, Firecrawl, Ahrefs, SE Ranking,
   Profound, Bing Webmaster, Unlighthouse. Use these for what the MCP does not
   expose, such as merchant and marketplace data for `/seo ecommerce`.

Never present an estimate as measured data, and never fill a gap with a guess.
Write `unknown` and name the source that would fill it.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Built by ahmadaimee → https://github.com/ahmadaimee/seo-agent
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
This skill orchestrates 30 sub-skills. The orchestrator itself (`seo`) is the 31st
in `skills/`, but does not orchestrate itself, so it is not enumerated below.

Skills marked **(MCP)** need a connected S.E.O Agent instance for their data;
everything else runs on the bundled Python toolchain.
...
25. **seo-keywords** -- Keyword research: discovery, volume, difficulty, intent, SERP validation, rank tracking **(MCP)**
26. **seo-competitor** -- One competitor's organic footprint, ranking keywords, content themes, backlinks, gaps **(MCP)**
27. **seo-landscape** -- Market-level view: leaders, winning themes, keyword coverage, strategic gaps **(MCP)**
28. **seo-link-prospecting** -- Link prospects, contact discovery, outreach drafts **(MCP)**
29. **seo-project-setup** -- One interview that fills the shared project context every skill reads **(MCP)**
30. **seo-coach** -- Beginner-friendly coach mode: explains the workflows and picks the next step
