...
1. Run S.E.O Agent locally (from the repository root):
   ...
   The app is at `http://localhost:3010` and the MCP server at `http://localhost:3010/mcp`. Auth is disabled (`AUTH_MODE=local_noauth`) — keep it bound to localhost or behind your own auth.
