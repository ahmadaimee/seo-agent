...
# Sibling skills the audit hands off to rather than spawning. They run as their
# own workflow after the audit, so they carry no `maxTurns` budget and have no
# agents/ file. Every other `seo-*` name in the skill must be a real subagent.
HANDOFF_SKILLS = {"seo-keywords"}


def _discover_audit_agents() -> set[str]:
    text = AUDIT_SKILL.read_text(encoding="utf-8")
    return set(re.findall(r"`(seo-[a-z-]+)`", text)) - HANDOFF_SKILLS
