...
### `/seo keywords <seed topic or domain>`

Keyword research and rank tracking. **Requires a connected S.E.O Agent instance**
(DataForSEO-backed, spends credits).

Turns seed topics into a prioritized opportunity set: discovery via
`research_keywords`, hydration with volume, difficulty, intent and CPC via
`get_keyword_metrics`, SERP validation for ambiguous terms, and Search Console
striking-distance queries (position 5-20) when GSC is connected. Also sets up and
runs rank trackers.

Output: the best opportunity theme, keywords to target now, and a table of
keyword, intent, volume, difficulty, CPC, priority and notes. Asks before saving
keywords.

Hand off to `/seo cluster` to map the shortlist onto pages.

### `/seo cluster map-pages <keywords>`

Maps an existing keyword list onto existing or proposed pages, and flags
cannibalization where two pages chase one intent. See `/seo cluster` above for the
full clustering workflow.

### `/seo competitor <domain>`

Deep dive on one competitor. **Requires a connected S.E.O Agent instance.**

Organic footprint and keyword count, exact ranking keywords with URL, rank,
intent and SERP type, content themes grouped by page type, backlink profile, and
head-to-head SERPs for shared terms. Ends with what they do well, where they are
vulnerable, which pages and keywords to pursue, and what not to copy.

Use `/seo landscape` first when the competitor set itself is unclear.

### `/seo landscape <topic or domain>`

Market-level view across several competitors. **Requires a connected S.E.O Agent
instance.**

Builds a representative query set, identifies recurring domains via
`find_serp_competitors`, groups them by type (direct competitor, publisher,
marketplace, community, documentation), sizes the leaders, and reports winning
content formats, theme gaps and authority advantages.

Output names the market leaders, the most winnable opportunity area and the
biggest barrier to ranking.

### `/seo link-prospecting <url or asset>`

Finds pages, sites and authors that might reference a linkable asset, discovers
contact paths, and drafts outreach. **Requires a connected S.E.O Agent instance**
for prospect discovery; contact discovery uses web search and page fetches.

Builds 5-10 prospecting queries from the asset, mines competitor backlink
profiles, filters for topical and editorial fit, assigns an outreach angle per
prospect, then finds the contact path and drafts the message.

Never invents an email address or handle. Every contact detail carries its source
URL.

Pair with `/seo backlinks` for the profile analysis that comes first.

### `/seo project-setup [domain]`

One interview that fills the project's shared context: site scope, goals,
positioning, writing preferences, competitors and key pages, plus Google Search
Console intake. **Requires a connected S.E.O Agent instance** to store anything.

Every other skill reads this context before working and writes back what is
durable, so the knowledge survives new sessions, machines and agents. The user can
edit it on the project's Context settings page. The context tools are free.

See `docs/PROJECT-CONTEXT.md` for the contract and the patch ops.

### `/seo coach`

Beginner-friendly coach mode. Explains what each workflow does, asks what the user
is trying to achieve, and recommends one next step rather than a menu. Adapts
depth to whether the user is new to SEO.
...
| `/seo keywords <seed>` | Keyword research, volume, difficulty, intent, rank tracking (needs S.E.O Agent) |
| `/seo competitor <domain>` | One competitor's footprint, keywords, themes, gaps (needs S.E.O Agent) |
| `/seo landscape <topic>` | Market leaders, winning themes, strategic gaps (needs S.E.O Agent) |
| `/seo link-prospecting <url>` | Link prospects, contact paths, outreach drafts (needs S.E.O Agent) |
| `/seo project-setup [domain]` | Fill the shared project context once (needs S.E.O Agent) |
