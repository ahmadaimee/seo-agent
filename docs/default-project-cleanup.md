...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent
   ```

2. If the output looks right, apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent --apply --confirm-remote-apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent --validate-only
   ```
...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent --local
   ```

2. Apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent --local --apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database seo-agent --local --validate-only
   ```
