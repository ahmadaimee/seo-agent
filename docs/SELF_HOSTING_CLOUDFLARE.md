...
Host S.E.O Agent on Cloudflare for internet-facing self-hosting across multiple devices or with your team. One deploy command provisions everything, including the Cloudflare Access login gate. Works on Cloudflare's free plan.
...
## 1) Clone your S.E.O Agent repo

Fork `ahmadaimee/seo-agent` on GitHub if you want a repo you control, then clone it locally:

```bash
git clone https://github.com/YOUR_GITHUB_USER/seo-agent.git
cd seo-agent
corepack enable
pnpm install
```
...
```bash
git clone https://github.com/ahmadaimee/seo-agent.git
cd seo-agent
corepack enable
pnpm install
```
...
3. S.E.O Agent should load after login.

If it doesn't, see Troubleshooting below.

## Updating to the latest S.E.O Agent version
...
- For server errors, open the Worker `Logs` or run `pnpm exec wrangler tail`. Site audits run in a separate worker: `pnpm exec wrangler tail seo-agent-selfhost-audit`.
