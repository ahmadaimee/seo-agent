...
Connecting Google Search Console (GSC) lets S.E.O Agent pull your real clicks,
impressions, positions, and URL inspection data, straight from Google.

It's **optional**: S.E.O Agent runs fine without it, just without Search Console data.
...
2. Add an **Authorized redirect URI** that exactly matches your deployment's
   ...
   | Local Docker | `http://localhost:3010/api/gsc/oauth/callback`            |
...
Set these three values, then restart S.E.O Agent:
...
- S.E.O Agent uses your Google client to run the OAuth flow and stores the resulting
...
- Search Console data comes from your own Google account, so S.E.O Agent never meters credits for it.
