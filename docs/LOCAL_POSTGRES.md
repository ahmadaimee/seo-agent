# Running S.E.O Agent on Postgres locally

S.E.O Agent runs on **Cloudflare D1 (SQLite) by default**. Postgres is an opt-in
...
```sh
docker run --name seoagent-postgres \
  -e POSTGRES_USER=seoagent \
  -e POSTGRES_PASSWORD=seoagent \
...
```sh
docker exec seoagent-postgres pg_isready -U seoagent -d seoagent
```

The connection string is:

```
postgres://seoagent:seoagent@localhost:5433/seoagent
```
...
```sh
POSTGRES_DATABASE_URL=postgres://seoagent:seoagent@localhost:5433/seoagent \
  pnpm db:migrate:pg
```
...
docker exec seoagent-postgres psql -U seoagent -d seoagent -c "\dt"

# Inspect rows the app writes (e.g. after creating a project / saving keywords)
docker exec seoagent-postgres psql -U seoagent -d seoagent -c "select count(*) from projects;"
```
